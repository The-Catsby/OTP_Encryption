 ...
 * Description: This program connects to otp_enc_d, and asks it to 
	* perform a one-time pad style encryption as detailed above. 
	* By itself, otp_enc doesn’t do the encryption.
******************************************************************/
...
int main(int argc, char *argv[])
{
	...
	{
		char errmsg[100];
		snprintf(errmsg, 100, "Error: could not contact otp_enc_d on port %i", portno);
		error(errmsg);
		exit(2);
	}
	
	//write to host: our process name 
	char procName[] = "otp_enc";
	...
	{
		char errmsg[100];
		snprintf(errmsg, 100, "ERROR: otp_enc cannot connect to otp_dec_d denied connection on port %i", portno);
		error(errmsg);
		exit(1);
	}

	/*	Open and send PLAINTEXT file string to otp_enc_d	*/
	fptr = fopen(argv[1], "r");
		...
		if(buffer)
		{
			...
			for(i=0; i < size; i++)
			{
				...
				else if(buffer[i] < 'A' || buffer[i] > 'Z')		
				{
					...
					error("otp_enc error: input contains bad characters, exiting");
					exit(1);
				}
			}
